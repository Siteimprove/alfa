...
>({
  ...
  tags: [Scope.Component, Version.of(2), Stability.Experimental],
  evaluate({ device, document }) {
    return {
      ...
      expectations(target) {
        const askGoodClasses = Question.of(
        ...
        return {
          1: askGoodClasses
            .map((goodClasses) =>
              Iterable.some(goodClasses, (goodClass) =>
                target.classes.includes(goodClass)
              )
            )
            .map((hasGoodClass) =>
