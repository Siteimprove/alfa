...
import { WithRole } from "../common/diagnostic.ts";
      ...
      expectations(target) {
        ...
        if (role.isSome()) {
          ...
          const inputType = target
            ...
            .map((attr) => attr.value)
            .getUnsafe(
              `R8v2 found an element with no role nor 'type' attribute: ${target.path()}`,
            ) as Element.InputType;
          return {
            1: expectation(
              hasNonEmptyAccessibleName(device)(target),
...
export namespace Outcomes {
  ...
  export const InputElementWithNoAriaRoleHasName = (
    typeAttribValue: Element.InputType,
  ) =>
    Ok.of(
      Diagnostic.of(
        `The type="${typeAttribValue}" form field has an accessible name`,
      ),
    );

  export const InputElementWithNoAriaRoleHasNoName = (
    typeAttribValue: Element.InputType,
  ) =>
    Err.of(
      Diagnostic.of(
        `The type="${typeAttribValue}" form field does not have an accessible name`,
      ),
    );
}
