...
test("evaluate() correctly resolves relative URLs", async (t) => {
  ...
  const iframe2 = <iframe title="Foo" src="path/to/foo.html" />;
  const iframe3 = <iframe title="Foo" src="/path/to/foo.html" />;
  const iframe4 = <iframe title="Foo" src="./foo.html" />;
  const iframe5 = <iframe title="Foo" src="down/../foo.html" />;
  const iframe6 = <iframe title="Foo" src="../to/foo.html" />;

  const iframe = iframe6;
  ...
  t.deepEqual(
    await evaluate(R15, {
      document,
      response: Response.of(URL.parse("https://somewhere.com").get(), 200),
    }),
    [passed(R15, [iframe1, iframe], { 1: Outcomes.EmbedSameResources })]
  );
});
