...
export namespace Parser {
  ...
  function exclusivePeek<I, T, E, A extends Array<unknown> = []>(
    pairs: Array<
      [peeker: Parser<I, unknown, unknown, A>, parser: Parser<I, T, E, A>]
    >,
    ifNoPeek: Mapper<I, E, A>,
  ): Parser<I, T, E, A> {
    return (input, ...args) => {
      ...
      return Err.of(ifNoPeek(input, ...args));
    };
  }

  function exclusivePredicate<I, T, U, E, A extends Array<unknown> = []>(
    peeker: Parser<I, U, unknown, A>,
    pairs: Array<[Predicate<U>, Parser<I, T, E, A>]>,
    ifNoMatch: Mapper<I, E, A>,
    ifNoPeek: Mapper<I, E, A> = ifNoMatch,
  ): Parser<I, T, E, A> {
    return (input, ...args) => {
      ...
      return Err.of(ifNoMatch(input, ...args));
    };
  }

  /**
   * Parses with the first parser that succeeds its peeker; or ifNoPeek if none.
   * Use when there are many parsers with short differentiating prefixes of
   * different nature, e.g. keywords vs symbols vs numbers.
   *
   * @remarks
   * Peekers results are discarded; the full input is given to the corresponding
   * parser.
   *
   * If a peeker succeeds, its corresponding parser is used, regardless of
   * whether the parser itself would succeed or not. Therefore, peekers should
   * be mutually exclusive.
   *
   * This allows to avoid backtracking when trying multiple parsers with no
   * prefix overlap. For example, suppose parseFoo accepts "foo(<number>, <number>)",
   * and similarly for parseBar. Then parsing "foo(1)" with
   * `either(parseFoo, parseBar)` will fail parseFoo and then try parseBar, even
   * though it is guaranteed to fail as well because of the prefix "foo".
   * However, `exclusive([[peekFoo, parseFoo], [peekBar, parseBar]], ifNoPeek)`
   * will only try parseFoo, because peekFoo succeeds, and avoid the unnecessary
   * attempt at parseBar.
   *
   * @public
   */
  export function exclusive<I, T, E, A extends Array<unknown> = []>(
    pairs: Array<
      [peeker: Parser<I, unknown, unknown, A>, parser: Parser<I, T, E, A>]
    >,
    ifNoPeek: Mapper<I, E, A>,
  ...
  /**
   * Parses with the first parser whose predicate matches the value
   * returned by the peeker; or ifNoMatch if none. Use when there are many
   * parsers with short differentiating prefixes of similar nature, e.g.
   * parsing functions differently based on their name.
   *
   * @remarks
   * Peeker results are discarded; the full input is given to the corresponding
   * parser.
   *
   * If a predicate matches, its corresponding parser is used, regardless of
   * whether the parser itself would succeed or not. Therefore, predicates should
   * be mutually exclusive.
   *
   * This allows to avoid backtracking when trying multiple parsers with no
   * prefix overlap. For example, suppose parseFoo accepts "foo(<number>, <number>)",
   ...
   * `either(parseFoo, parseBar)` will fail parseFoo and then try parseBar, even
   * though it is guaranteed to fail as well because of the prefix "foo".
   * However, `exclusive(peekName, [[isFoo, parseFoo], [isBar, parseBar]], ifNoMatch)`
   * will only try parseFoo, because isFoo matches, and avoid the unnecessary
   * attempt at parseBar.
   *
   * @public
   */
  ...
  export function exclusive<I, T, U, E, A extends Array<unknown> = []>(
    peekerOrPairs:
      | Parser<I, U, unknown, A>
      | Array<[Parser<I, unknown, unknown, A>, Parser<I, T, E, A>]>,
    pairsOrIfErr: Array<[Predicate<U>, Parser<I, T, E, A>]> | Mapper<I, E, A>,
    NoMatch?: Mapper<I, E, A>,
    ifNoPeek?: Mapper<I, E, A>,
  ): Parser<I, T, E, A> {
    if (typeof peekerOrPairs === "function") {
      return exclusivePredicate(
        peekerOrPairs,
        pairsOrIfErr as Array<[Predicate<U>, Parser<I, T, E, A>]>,
        NoMatch!,
        ifNoPeek,
      );
    } else {
      return exclusivePeek(
        peekerOrPairs as Array<
          [Parser<I, unknown, unknown, A>, Parser<I, T, E, A>]
        >,
        pairsOrIfErr as Mapper<I, E, A>,
      );
    }
  }
