...
/**
 * Helpers for gamma conversions between RGB spaces.
 */
namespace Gamma {
  /**
   * Fake gamma conversion for XYZ spaces
   */
  export function xyz(value: number): number {
    return value;
  }
   ...
   * gamma correction.
   */
  export function decoding(value: number): number {
   ...
   * gamma correction.
   */
  export function encoding(value: number): number {
    let sign = value < 0 ? -1 : 1;
    let abs = Math.abs(value);
...
export namespace ColorSpace {
  ...
  const spaces: { [key in ColorSpace]: RGBColorSpace } = {
    "a98-rgb": {
      whitepoint: "D65",
       ...
       */
      gammaDecoding: Gamma.correction(563 / 256),
       ...
       */
      gammaEncoding: Gamma.correction(256 / 563),
    ...
    "prophoto-rgb": {
       ...
       */
      gammaDecoding: Gamma.correction(
        1.8,
        (value) => value <= 16 / 512,
        1 / 16,
      ),
       ...
       */
      gammaEncoding: Gamma.correction(1 / 1.8, (value) => value < 1 / 512, 16),
    ...
    rec2020: {
       ...
       */
      gammaDecoding: Gamma.correction(2.4),
       ...
       * If this causes problems, we will need to revisit this.
       */
      gammaEncoding: Gamma.correction(1 / 2.4),
