import * as enquirer from "enquirer";

import { Future } from "@siteimprove/alfa-future";
import { Iterable } from "@siteimprove/alfa-iterable";
import { Option, None } from "@siteimprove/alfa-option";
import { Question } from "@siteimprove/alfa-rules";
import { Page } from "@siteimprove/alfa-web";

import * as act from "@siteimprove/alfa-act";
import * as xpath from "@siteimprove/alfa-xpath";

/**
 * @internal
 *
 * @todo Make this stuff external to the CLI so that it simply requires a package
 *       responsible for doing the interview.
 */
export const Oracle = (page: Page): act.Oracle<Question> => {
  const answers = new Map<unknown, Map<string, Future<any>>>();

  return (_, question) => {
    let answersForSubject = answers.get(question.subject);

    if (answersForSubject === undefined) {
      answersForSubject = new Map();
      answers.set(question.subject, answersForSubject);
    }

    let answer = answersForSubject.get(question.uri);

    if (answer === undefined) {
      process.stdout.write(`\n${question.subject}\n\n`);

      switch (question.type) {
        case "boolean": {
          answer = Future.from(
            enquirer
              .prompt<{ [key: string]: boolean }>({
                name: question.uri,
                type: "toggle",
                message: question.message,
              })
              .then((answer) =>
                Option.of(question.answer(answer[question.uri]))
              )
              .catch(() => None)
          );
          break;
        }

        case "node": {
          answer = Future.from(
            enquirer
              .prompt<{ [key: string]: string }>({
                name: question.uri,
                type: "input",
                message: question.message,
                validate: (expression) => {
                  if (expression === "") {
                    return true;
                  }

                  const nodes = [
                    ...xpath.evaluate(page.document, expression, {
                      composed: true,
                      nested: true,
                    }),
                  ];

                  if (nodes.length === 1) {
                    return true;
                  }

                  return "Invalid XPath expression";
                },
              })
              .then((answer) => {
                const expression = answer[question.uri];

                const node = Iterable.first(
                  xpath.evaluate(page.document, expression, {
                    composed: true,
                    nested: true,
                  })
                );

                return Option.of(question.answer(node));
              })
              .catch(() => None)
          );
          break;
        }
      }

      answer = Future.now(None);
      answersForSubject.set(question.uri, answer);
    }

    return answer;
  };
};
