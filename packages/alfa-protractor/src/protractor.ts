...
export namespace Protractor {
  ...
  export async function toPage(value: Type): Promise<Page> {
    const element: Element.JSON = await value.browser_.executeScript(
      (element: globalThis.Element) => {
        ...
        function toStyleRule(
          styleRule: globalThis.CSSStyleRule
        ): StyleRule.JSON {
          return {
            type: "style",
        ...
        function toImportRule(rule: globalThis.CSSImportRule): ImportRule.JSON {
          return {
            ...
            condition:
              rule.media.mediaText === "" ? "all" : rule.media.mediaText,
            href: rule.href,
          };
        }
