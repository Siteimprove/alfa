import { isQuoted, removeFirstScopedDirectory, toPascalCase, } from '../libs/utils/index.js';
import { getHierarchyRoots } from '../theme/lib/index.js';
import path from 'path';
import { BaseRouter, EntryPointStrategy, PageKind, Reflection, ReflectionKind, Slugger, } from 'typedoc';
export class MarkdownRouter extends BaseRouter {
    ...
    buildAnchors(target, pageTarget) {
        ...
        if (!target.kindOf(ReflectionKind.TypeLiteral)) {
            ...
            while (refl.parent && refl.parent !== pageTarget) {
                ...
                if (!refl.kindOf(ReflectionKind.TypeLiteral | ReflectionKind.FunctionOrMethod)) {
            ...
            // --------------------------------------------
            if (target.kindOf(ReflectionKind.CallSignature) &&
                target.parent?.kindOf(ReflectionKind.Method)) {
                return;
            }
            let toSlug = parts.join('.');
            if (this.tableAnchorRules.some((r) => this.isTableAnchor(target, r.targetKind, r.parentKind, r.option))) {
    ...
    isTableAnchor(target, targetKind, parentKind, option) {
        return (!!target.kindOf(targetKind) &&
            !!target.parent?.kindOf(parentKind) &&
            this.application.options.getValue(option)
                .toLowerCase()
                .endsWith('table'));
    }
