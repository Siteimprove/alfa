# ADR 10: [CANCELLED] Use `Future<T>` to model possibly asynchronous computations 
...
Cancelled. Modern promises, especially with the possibility of top-level `await`, made the need for `Future` much smaller, and the added maintenance burden is not worth anymore.
