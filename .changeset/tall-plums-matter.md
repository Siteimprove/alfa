...
**Breaking:** Used values of style properties can now be any type, wrapped in a value, not just an `Option` of the computed type. The `Style.hasUsedValue` predicate now requires as argument a predicate of the used value type (instead of the computed one).
